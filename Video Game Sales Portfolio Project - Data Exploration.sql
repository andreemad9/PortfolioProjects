...
SELECT 
  Genre, SUM(Global_Sales) AS TotalSales
FROM 
  vgsales
GROUP BY 
  Genre
...
SELECT 
  Year, AVG(Global_Sales) AS Average_Sales 
FROM 
  vgsales 
GROUP BY 
  Year
...
SELECT 
  Publisher, ROUND(SUM(Global_Sales), 2) AS TotalSales
FROM 
  vgsales
GROUP BY 
  Publisher
...
SELECT 
  Genre, ROUND(AVG(Global_Sales), 2) AS AverageSales
FROM 
  vgsales
GROUP BY 
  Genre
...
FROM 
	vgsales
WHERE 
	Global_Sales > 1.0 AND Year >= 2010;
